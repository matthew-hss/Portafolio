                ...
                if (lastSaleItem.getProduct() != null && lastSaleItem.getQuantity() > 0) {
                    ...
                    if (!voucherCode.trim().equals("")) {
                        ...
                        if (voucherType.equals("111")) {
                            ...
                            specialVoucher = voucherService.getSpecialVoucher(voucherCode);
                            if (specialVoucher != null && specialVoucher.getSale() == null) {
                                sale.setSpecialVoucher(specialVoucher);
                                total -= Integer.parseInt(specialVoucher.getAmount().toString());
                                if (total < 0) {
                                    total = 0;
                                }
                                sale.setTotal(Long.parseLong(total.toString()));
                                Sale save = saleService.save(sale);
                                if (save != null) {
                                    specialVoucher.setSale(save);
                                    specialVoucher.setUsed(true);
                                    SpecialVoucher specialVoucherUpdated = voucherService.save(specialVoucher);
                                    if (specialVoucherUpdated != null) {
                                        FacesUtils.infoMessage("specialVoucherUsed");
                                    }
                                    SaleItem saveItem = null;
                                    for (SaleItem s : saleItems) {
                                        s.setSale(save);
